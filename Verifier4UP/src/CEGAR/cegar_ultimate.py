...
from Verifier4UP.src.CEGAR.auxiliary_ultimate import translate_albet_to_stat_save, translate_albet_to_stat_print, check_empty, update_automata
...
def trace_refinement(ats, bpl, time_limit=1200, print_flag=False):
    ...
    # ------------------------------------------------------------------------------------------- #
    # ------------------------------------ the location file output ----------------------------- #
    # ------------------------------------------------------------------------------------------- #
    # 初始自动机文件
    with open("./check_empty_automata.ats", "w") as f:
        f.write(ats)
        f.write("\nFiniteAutomaton intersecNfa = FSM;\n")
        f.write("print(isEmpty(intersecNfa));\n")
        f.write("Word word = getAcceptedWord(intersecNfa);\n")
        f.write("print(word);")
    ...
    [isEmpty_result, accepted_word_result] = check_empty("./check_empty_automata.ats", ultimate_info=False)
    ...
    while isEmpty_result == "false" and ((end - begin) < time_limit):
        ...
        if generalized_result == False:  # found a feasible error trace
            ...
            update_automata("./check_empty_automata.ats", ats, refinement_num)
